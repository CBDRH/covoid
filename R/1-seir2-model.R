...
#' Simulate a deterministic SEIR2 model
#'
#' @description
...
#'
#' @examples
#' param <- seir2_param(R0 = 2.5,gamma = 0.1,sigma=0.1)
#' state0 <- seir2_state0(S = 100,E = 1, I = 0,R = 0)
...
#' else 1.1
#' }
#' param <- seir2_param(R0 = R_t,gamma = 0.1,sigma=0.1)
#' state0 <- seir2_state0(S = 100,E = 1, I = 0,R = 0)
#' res <- simulate_seir2(t = 100,state_t0 = state0,param = param)
#' plot(res,c("S","E","I","R"))
#'
...
#'
# seir2_param <- function(R0,beta,sigma,gamma) {
...
#'
# seir2_state0 <- function(S,E,I=0,R=0) {
#     # assertions
#     stopifnot(E >= 1)
#
...
#' SEIR2 differential equations
#'
#' @param t Number of time steps over which to sample from the model
#' @param state_t0 Initial state (S,E,I,R)
#' @param param The model parameters (R0,beta,sigma,gamma)
#'
#' @return derivatives of SEIR2 model states with respect to time
#'
