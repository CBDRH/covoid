...
\lambda_i &= \sum{p^T \cdot IM_{*,i} \cdot CM_{i,*} \cdot \frac{E\it{2} + I1m + I1s + I2}{N}} \\
          ...
          &+ (1 - T_{eff}) \cdot \sum{p^T \cdot IM_{*,i} \cdot CM_{i,*} \cdot \frac{T}{N}} \\
...
$$
\begin{aligned} 
\frac{dS_i}{dt} &= -S_{i} \cdot \lambda_{i} - \kappa_i \cdot (1 -\alpha) \cdot \lambda_i \cdot S_i + \eta \cdot S^C_i \\
\frac{dE\it{1}_i}{dt} &= (1 - \delta_1) \cdot S_{i} \cdot \lambda_{i} - \sigma_1 \cdot E\it{1}_i \\
\frac{dE\it{2}_i}{dt} &= (1 - \delta_2) \cdot \sigma_1 \cdot E\it{1}_i - \sigma_2 \cdot E2_i \\
\frac{dI\it{1}m_i}{dt} &= (1 - \delta_3) \cdot (1 - p_{severe}) \cdot \sigma_2 \cdot E2_i  - \gamma_1 \cdot I\it{1}m_1 \\
\frac{dI\it{1}s_i}{dt} &= (1 - \delta_3) \cdot p_{severe} \cdot \sigma_2 \cdot E2_i  - \gamma_1 \cdot I\it{1}s_i \\
\frac{dI\it{2}_i}{dt} &= (1 - \delta_4) \cdot (1 - p_{isolate}) \cdot \gamma_1 \cdot I\it{1}m_i  - \gamma_2 \cdot I\it{2}_i \\
\frac{dT_i}{dt} &= (1 - \delta_4) \cdot p_{isolate} \cdot \gamma_1 \cdot I\it{1}m_i  - \gamma_2 \cdot T_i \\
\frac{dH_i}{dt} &= \gamma_1 \cdot I\it{1}s_i  - \gamma_2 \cdot H_i \\
\frac{S^C_i}{dt} &= \kappa_i \cdot (1 -\alpha) \cdot \lambda_i \cdot S_i - \eta \cdot S^C_i\\
\frac{dE\it{1}^C_i}{dt} &= \delta_1 \cdot S_{i} \cdot \lambda_{i} - \sigma_1 \cdot E\it{1}^C_i \\
\frac{dE\it{2}^C_i}{dt} &= \delta_2 \cdot \sigma_1 \cdot E\it{1}_i + \sigma_1 \cdot E\it{1}^C_i - \sigma_2 \cdot E2^C_i \\
\frac{dI\it{1}m^C_i}{dt} &= \delta_{3} \cdot (1-p_{severe}) \cdot \sigma_2 \cdot E2_i + (1 - p_{severe}) \cdot \sigma_2 \cdot E2^C_i  - \gamma_1 \cdot I\it{1}m^C_1 \\
\frac{dI\it{1}s^C_i}{dt} &= \delta_{3} \cdot (p_{severe}) \cdot \sigma_2 E2_i + p_{severe} \cdot \sigma_2 \cdot E2^C_i  - \gamma_1 \cdot I\it{1}s^C_i \\
\frac{dI2^C_i}{dt} &= \delta_{4} \cdot \gamma_1 \cdot I1m_i + \gamma_1\cdot I1m^C_i - \gamma_2 \cdot I2^C_i \\
