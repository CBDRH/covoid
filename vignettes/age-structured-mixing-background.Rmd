...
knitr::kable(cm,
             ...
             caption="Source: @blackwood2018introduction")
```
...
\end{align}
$$
This give a characteristic equation with solutions $\lambda_{\pm} = \frac{1}{2}[\frac{\beta}{\gamma}(k_{11} + k_{22}) \pm \beta \sqrt{4f_1f_2\frac{1}{\gamma}k_{12}k_{21} + \frac{1}{\gamma^2} (k_{11}^2 + k_{22}^2 - 2  k_{11} k_{22} )}]$, where $f_1 = \frac{p_1}{p_2}$ and $f_2 = \frac{p_2}{p_1}$. While an unwieldy equation, notice that if $f_1 = f_2$ and $\kappa_{ij} = \kappa_{ji}$ for all i,j pairs then it reduces to the familiar $\frac{\beta\kappa}{\gamma}$ for the homogeneous SIR model. Notice the role of the population weights p and the comparison with the R0 for a homogeneous SIR model of beta/gamma. We can also numerically solve for $\mathcal{R_0}$, e.g. using the following R script:  
