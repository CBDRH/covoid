...
```{r,fig.height=3.5,fig.width=7,fig.retina=3,fig.cap="*Figure 5*: Epidemic trajectory"}
cm <- matrix(c(5.8,1.3,2.6,1.9),ncol=2,byrow = TRUE)
dist <- c(1/3,2/3)
...
knitr::kable(cm,
             row.names = TRUE,
             col.names = c("Individual: child","Individual: adult"),
             caption=expression("Varying the contact rate, the $\\alpha$ parameter varies from 0 to 1 (Source: Blackwood and Childs, 2018)"))
```
...
The basic reproduction number $\mathcal{R_0}$ is defined as the average number of secondary infections arising from a single individual during their entire infectious period, in a population of susceptibles (Heffernan, Smith, & Wahl, 2005). In many models (including those under discussion) $\mathcal{R_0}$ is related to the peak and final spread of a epidemic, and acts as a threshold parameter for invasion of a disease organism into a completely susceptible population (Driessche & Watmough). However, for complex models there is not a uniformly accepted mathod for calculating $\mathcal{R_0}$. As shown by Li et al (2010) the various methods give different answers. With this qualification we descibe the next generation approach to calculation of $\mathcal{R_0}$. This approach is used by in COVOID package and other xxx (Prem et al, 2020) to calculate $\mathcal{R_0}$ and/or translate $\mathcal{R_0}$ into more basic quantities.
...
We will consider the generalised SEIR case, in which $E$ and $I$ compartments may be further decomposed into $E_1,...,E_n$ and $I_1,...,I_n$. Changing nomenclature a little, let $y \in \mathbb{R}^m$ be m-vector of nondisease compartments (e.g. S) and $x$ the disease compartments. For example $y=S$ and $x=\{E,I\}$. Note that we can ignore R for the case of no reinfection as it simply acts as an end-state reservoir. The time derivative of x $x_i' =\mathcal{F_i}(x) - \mathcal{V_i}(x)$, where $\mathcal{F}(x)$ is the rate secondary infections increase the $i^{th}$ compartment and $\mathcal{V}$ is the rate of disease progression. We then construct the matrices $F$ with elements $\frac{\partial \mathcal{F_i}}{\partial x_i}(x_0)$ and $V$ with elements $\frac{\partial \mathcal{V_i}}{\partial x_i}(0)$. The state vector $x_0$ is a vector of zeros refered to as the disease free equilibrium, the point at which no disease is present in the population (Morris, 2010). The next generation matrix $K = FV^{-1}$, with $\mathcal{R_0} = \rho(K)$ where $\rho(K)$ is the spectral radious of maximum of the absolute values of the eigenvalues of $K$. For example using the contact matrix and population distribution information from the SIR model

$$
\begin{align}
\mathcal{F_1} &= S_1(\beta \kappa_{11} \frac{I_1}{N_1} + \beta \kappa_{12} \frac{I_2}{N_2}) \\
\mathcal{F_2} &= S_2(\beta \kappa_{21} \frac{I_1}{N_1} + \beta \kappa_{22} \frac{I_2}{N_2}) \\
\\
F &= \begin{pmatrix}
...
\end{pmatrix} \\
 &= \begin{pmatrix}
 \beta \kappa_{12} & \frac{p_1}{p_2} \beta \kappa_{12} \\
\frac{p_2}{p_1} \beta \kappa_{21}  &  \beta \kappa_{22} \\
\end{pmatrix}
\end{align}
$$
Where we remind the reader that $p_j = \frac{N_j}{N}$, as $S_j = N_j$ since disease free equilibrium. Since $\mathcal{V} = $ its derivative with respect to ... is just $\gamma$ we have that $V = \gamma I_n$, where $I_n$ is an $n \times n$ identity matrix, and $V^{-1} = \frac{1}{\gamma} I_n$. Multiply $F$ and $V^{-1}$ gives
...
\end{align}
$$
This give a characteristic equation with solutions $\lambda_{\pm} = \frac{1}{2}[\frac{\beta}{\gamma}(k_{11} + k_{22}) \pm \beta \sqrt{4f_1f_2\frac{1}{\gamma}k_{12}k_{21} + \frac{1}{\gamma^2} (k_{11}^2 + k_{22}^2 - 2  k_{11} k_{22} )}]$, where $f_1 = \frac{p_1}{p_2}$ and $f_2 = \frac{p_2}{p_1}$. While an unwieldy equation, notice that if $f_1 = f_2$ and $\kappa_{ij} = \kappa_{ji}$ for all i,j pairs then it reduces to the familiar $\frac{\beta\kappa}{\gamma}$ for the homogeneous SIR model. Notice the role of the population weights p and the comparison with the R0 for a homogeneous SIR model of beta/gamma. We can also numerically solve for $\mathcal{R_0}$ as
...
Armbruster, B., & Beck, E. (2017). Elementary proof of convergence to the mean-field model for the SIR process. Journal of mathematical biology, 75(2), 327-339.
...
Davies, N. G., Klepac, P., Liu, Y., Prem, K., Jit, M., Eggo, R. M., & CMMID COVID-19 working group. (2020). Age-dependent effects in the transmission and control of COVID-19 epidemics. MedRxiv.
...
Mistry, D., Litvinova, M., Chinazzi, M., Fumanelli, L., Gomes, M. F., Haque, S. A., ... & Longini Jr, I. M. (2020). Inferring high-resolution human mixing patterns for disease modeling. arXiv preprint arXiv:2003.01214.

Morris, Q. (2010). Analysis of a Co-Epidemic Model.

Mossong, J., Hens, N., Jit, M., Beutels, P., Auranen, K., Mikolajczyk, R., ... & Heijne, J. (2008). Social contacts and mixing patterns relevant to the spread of infectious diseases. PLoS medicine, 5(3).

Ni, S., & Weng, W. (2009). Impact of travel patterns on epidemic dynamics in heterogeneous spatial metapopulation networks. Physical review E, 79(1), 016111.
...
Viboud, C., Boëlle, P. Y., Cauchemez, S., Lavenu, A., Valleron, A. J., Flahault, A., & Carrat, F. (2004). Risk factors of influenza transmission in households. British Journal of General Practice, 54(506), 684-689.

Wu, J. T., Leung, K., Bushman, M., Kishore, N., Niehus, R., de Salazar, P. M., ... & Leung, G. M. (2020). Estimating clinical severity of COVID-19 from the transmission dynamics in Wuhan, China. Nature Medicine, 26(4), 506-510.
