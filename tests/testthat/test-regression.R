...
test_that("models give same results for S_inf as in previous version (regression test)", {
    TT <- 1000
    # homogeneous models
    ...
    expect_equal(107.2767, res_sir$epi$S[TT],tolerance=0.1)
    expect_equal(107.2767, res_seir$epi$S[TT],tolerance=0.1)
    expect_equal(107.2767, res_seir1$epi$S[TT],tolerance=0.1)
    expect_equal(107.2767, res_seir2$epi$S[TT],tolerance=0.1)
    expect_equal(107.2767, res_sir_c$epi$S[TT],tolerance=0.1)
    expect_equal(107.2767, res_seir_c$epi$S[TT],tolerance=0.1)
})

test_that("models give same results for I_peak as in previous version (regression test)", {
    TT <- 1000
    # homogeneous models
    res_sir <- simulate_sir(t = TT,
                            state_t0 = sir_state0(S = 1000,I = 2,R = 0),
    ...
    res_seir2 <- simulate_seir2(t = TT,
                                ...
                                param = seir2_param(R0=2.5,lambdaimp=0,sigma1=0.1,sigma2=0.1,gamma1=0.1,
                                                    ...
                                                    alphamBeta=0,probHospGivenInf=0,delta=0,kappa=0,pm=0))
    # age structured models
    cm <- matrix(c(0.5,0.5,0.5,0.5),ncol=2,nrow=2)
    dist <- c(0.5,0.5)
    res_sir_c <- simulate_sir_c(t = TT,
                                state_t0 = sir_c_state0(S = c(500,500),I=c(1,1),R=c(0,0)),
    ...
    res_seir_c <- simulate_seir_c(t = TT,
                                  ...
                                  param = seir_c_param(R0=2.5,sigma=0.1,gamma=0.1,cm=cm,dist=dist))
    # compare to base SIR model
    expect_equal(45, which(res_sir$epi$I == max(res_sir$epi$I)),tolerance=0.1)
    expect_equal(117, which(res_seir$epi$I == max(res_seir$epi$I)),tolerance=0.1)
    expect_equal(189, which(res_seir1$epi$I == max(res_seir1$epi$I)),tolerance=0.1)
    expect_equal(189, which(res_seir2$epi$I == max(res_seir2$epi$I)),tolerance=0.1)
    expect_equal(45, which(res_sir_c$epi$I == max(res_sir_c$epi$I)),tolerance=0.1)
    expect_equal(117, which(res_seir_c$epi$I == max(res_seir_c$epi$I)),tolerance=0.1)
})
