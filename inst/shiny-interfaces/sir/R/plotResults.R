plotResults <- function(df, scale, logScale, plotvars, ndays, xtraC = NULL, xtraP = NULL) {
        ...
        if (xtraC!="") {
                x1 <- c('gray90')
                names(x1) = xtraC
                compcols <- c(c("S" = "lightblue", "I" = "red", "R" = "lightgreen"), x1)

                x2 <- if (xtraP=="") xtraC else xtraP
                names(x2) = xtraC
                complabels <- c(c("S" = "Susceptible", "I" = "Infectious", "R" = "Recovered"), x2)

                compbreaks <- c(c("S", "I", "R"), xtraC)
        }
        else {
        ...
        if(xtraC != "") {
                ...
                p <- p + geom_point_interactive(data=dx,
                                                aes(x=date, y=cases,
